 ...
 *
 * (01/Dec/2008) Mark Adamson
 *      Added support for the FT2232H and FT4232H chips.
 *
...
struct ftdi_private {
	...
	__u16 interface;	/* FT2232C, FT2232H or FT4232H port interface (0 for FT232/245) */
...
static __u32 ftdi_2232h_baud_base_to_divisor(int baud, int base)
{
	static const unsigned char divfrac[8] = { 0, 3, 2, 4, 1, 5, 6, 7 };
	__u32 divisor;
	int divisor3 = (base / 10 / baud) * 8; // hi-speed baud rate is 10-bit sampling instead of 16-bit
	divisor = divisor3 >> 3;
	divisor |= (__u32)divfrac[divisor3 & 0x7] << 14;
	/* Deal with special cases for highest baud rates. */
	if (divisor == 1) divisor = 0; else	// 1.0
...
static void ftdi_set_max_packet_size(struct usb_serial_port *port)
{
	...
	{
		info("Endpoint %d MaxPacketSize %d\n", i+1,
			interface->cur_altsetting->endpoint[i].desc.wMaxPacketSize);			
		ep_desc = &interface->cur_altsetting->endpoint[i].desc;
		if (ep_desc->wMaxPacketSize == 0) {
			ep_desc->wMaxPacketSize = cpu_to_le16(0x40);
			info("Overriding wMaxPacketSize on endpoint %d\n", i);
		}	
	}
...
static void ftdi_write_bulk_callback (struct urb *urb)
{
	...
	if (data_offset > 0) {
		...
		countback -= (data_offset * ((countback + (priv->max_packet_size - 1)) / priv->max_packet_size));
	}
	spin_lock_irqsave(&priv->tx_lock, flags);
	--priv->tx_outstanding_urbs;
...
{ /* ftdi_read_bulk_callback */
	...
	countread -= 2 * ((countread + (priv->max_packet_size - 1)) / priv->max_packet_size);
	spin_lock_irqsave(&priv->rx_lock, flags);
...
{ /* ftdi_process_read */
	...
	need_flip = 0;
	for (packet_offset = priv->rx_processed; packet_offset < urb->actual_length; packet_offset += priv->max_packet_size) {
		int length;
		...
		length = min((int)priv->max_packet_size, urb->actual_length-packet_offset)-2;
		if (length < 0) {
