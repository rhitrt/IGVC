...
// Uses 3 random points to construct a normal vector
bool ClearpathDemoTools::GetPlaneFromRnd3(PointCloud* cloud, double* normal_coeff)
{
    ...
    if (num > 3) //TODO timeout incase points are all close? 
    {
        Vector3 a1, a2, a3;
                              ...
                              unsigned int _numIncZ)
{
	...
	// Only initialize these if it's our first time through
	if (mode == MODE_NONE) {
...
void ClearpathDemoTools::VerticalCut(PointCloud* cloud, PointCloud* cloud_out, double min, double max)
{
    cloud_out->points.clear();
    int skim_factor = 13;
...
Twist ClearpathDemoTools::DetermineVelocity(double x, double z)
{
    Twist tw;
    tw.linear = lin_speed;
    tw.angular = 0.0;
...
// Calculate bins given a coordinate
...
Twist ClearpathDemoTools::Update(PointCloud* cloud, PointCloud* result)
{
    ...
    if (!normal_initialized)
    {
        ...
        VerticalCut(&cloudCopy, &cloudCut, base_normal[3]-0.1, base_normal[3]+0.1);
    ...
    if (mode == MODE_EXPLORE) {
        ...
        if (!clearpath) { // continue turning
            Twist tw;
            tw.linear = 0.0;
            tw.angular = ang_speed*dir;
            return tw;
        } else if (bestd[bestOverall] > 0.0 && fabs(bestxi[bestOverall]) < 1e-4) { // found a straight
            Twist tw;
            tw.linear = lin_speed;
            tw.angular = 0.0;
            return tw;
        } else if (bestd[bestOverall] > 0.0) { // found a target!
            Twist *tw = new Twist[numIncX];
            for (unsigned int i = 0; i < numIncX; i++)
            {
                tw[i] = DetermineVelocity(bestxi[i], bestzi[i]);
            }
            Twist besttw;
            besttw.linear = 0.0;
            besttw.angular = 0.0;
            for (unsigned int i = 0; i < numIncX; i++)
            {
                if (_copysign(1.0,tw[bestOverall].angular) == _copysign(1.0,tw[i].angular))
        ...
        if (bestc > 0) { // found a target!
            Twist tw;
            if (bestd > 1.0) {
                tw = DetermineVelocity(targetX, targetZ);
            } else {
                tw.linear = 0.0;
                double tempspd = ang_speed*std::min(fabs(bestx/0.35), 1.0);
                tw.angular = -1.0*_copysign(tempspd, bestx);
            }
            return tw;
        } else { // no target
            Twist tw;
